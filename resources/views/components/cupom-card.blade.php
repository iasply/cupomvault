...
>
        ...
        <div class="text-gray-600 text-sm space-y-1 mb-4">
            ...
            @if(!empty(@$cupom->dta_emissao_cupom))
                <p><strong>Emissão:</strong> {{ optional(Carbon::parse(@$cupom->dta_emissao_cupom ?? null))->format('d/m/Y') }}</p>
            @endif
            @if(!empty(@$cupom->dta_inicio_cupom))
                <p><strong>Início:</strong> {{ optional(Carbon::parse(@$cupom->dta_inicio_cupom ?? null))->format('d/m/Y') }}</p>
            @endif
            @if(!empty(@$cupom->dta_termino_cupom))
                <p><strong>Validade:</strong> {{ optional(Carbon::parse(@$cupom->dta_termino_cupom ?? null))->format('d/m/Y') }}</p>
            @endif
                ...
                `;
                    ...
                    <div class="overflow-y-auto max-h-96">
                        ...
                        switch(item.status.toLowerCase()) {
                            case 'utilizado': statusClass = 'bg-red-200 text-red-800'; break;
                            case 'não utilizado': statusClass = 'bg-blue-200 text-blue-800'; break;
                            case 'ativo': statusClass = 'bg-green-200 text-green-900'; break;
                            case 'vencido': statusClass = 'bg-yellow-200 text-yellow-900'; break;
